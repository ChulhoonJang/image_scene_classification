...
Even with significantly fewer parameters, the CNN-Baseline-best achieves near-perfect Top-2 accuracy across all classes. The confused class pairs such as glacier/mountain and buildings/street remain identical to those observed in ConvNeXt-Tiny-best, further confirming that the performance ceiling is determined by dataset ambiguity rather than model capacity. This is also consistent with the presence of mislabeled samples observed during error analysis.

The primary bottleneck in this dataset is not model capacity but label ambiguity. Classes such as glacier/mountain and buildings/street share overlapping visual characteristics that make hard boundaries difficult to define even for human annotators. This is evidenced by the consistent Top-2 accuracy above 99% across both models, as well as the presence of mislabeled samples identified during error analysis.